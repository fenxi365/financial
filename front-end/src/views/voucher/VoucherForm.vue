    ...
    <div class="h-panel" style="width: 1000px;margin: 0 auto;padding-top: 20px;">
      ...
      <div class="padding-right-left">
        ...
        <Row class="margin-top" type="flex" :space-x="10">
          ...
          <Cell class="label flex1">
            <div class="text-right">
              核算组织： <Select style="display: inline-block;min-width: 120px" disabled :deletable="false" :datas="orgDatas"
                            v-model="form.orgId" keyName="id" titleName="name"/>
            </div>
          </Cell>
        </Row>
      </div>
...
export default {
  ...
  computed: {
    ...mapState(['User', 'currentAccountSets','orgDatas','currentOrgId','currentOrg']),
    voucherItems() {
  ...
  methods: {
    ...
    before() {
      this.loading = true;
      this.$api.voucher.beforeId({currentId: this.voucherId,orgId:this.currentOrgId}).then(({data}) => {
        if (data) {
    ...
    next() {
      this.loading = true;
      this.$api.voucher.nextId({currentId: this.voucherId,orgId:this.currentOrgId}).then(({data}) => {
        if (data) {
          this.$router.push({name: 'VoucherForm', params: {voucherId: data}})
        } else {
    ...
    init() {
      if (this.voucherId) {
        ...
        this.form.voucherDate = moment(this.currentOrg.currentAccountDate).endOf('month').format('YYYY-MM-DD');
      }
...
/deep/ .h-select-disabled .h-select-show {
