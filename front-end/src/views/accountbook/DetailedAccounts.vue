<template>
  <app-content class="h-panel">
    <div class="h-panel-bar">
      <span class="h-panel-title"><template v-if="subject">{{ subject.name }}</template><template
          v-if="subject && subject.unit">(单位:{{ subject.unit }})</template> - 明细账 </span>
    </div>
    ...
    <Row class="h-panel-body" type="flex" :space-x="10">
      <Cell :flex="1" style="overflow-x: auto;">
        <table class="cus-table" v-if="!showNumPrice">
          ...
          <tr v-for="item in datalist" :key="item.subjecId">
            ...
            <td>
              <router-link v-if="item.voucherId" :to="{name:'VoucherForm',params:{voucherId:item.voucherId}}">
                {{ item.word }}-{{ item.code }}
              </router-link>
            </td>
      ...
      <Cell v-width="200">
        <Tree v-model="subjectId" @select="subjectSelect" :option="param" ref="subject" filterable selectOnClick
...
export default {
  ...
  computed: {
    ...mapState(['currentOrgId','orgDatas']),
  },
  watch: {
    subject() {
  ...
  methods: {
    loadSubject() {
      this.datalist = [];
      this.$api.accountbook.loadSubject({accountDate: this.accountDate,orgId:this.orgId}).then(({data}) => {
