<template>
  <Modal v-model="templateItemModel" hasCloseIcon hasDivider :closeOnMask="false">
    ...
    <Form v-width="700" mode="twocolumn" :labelWidth="90" ref="templateItemForm" :rules="validationTifRules"
          :model="templateItemForm">
    ...
    <div slot="footer">
      ...
      <Button color="red"  v-if="!templateItemForm.id" :loading="loading" @click="addItem(true)">保存并继续</Button>
      <Button color="primary" :loading="loading" @click="addItem(false)">保存</Button>
    </div>
  </Modal>
</template>
<script>
  ...
  methods: {
    ...
    addItem(go = false) {
      ...
      if (validResult.result) {
        ...
        this.loading = true;
        Api.report.template.items[this.templateItemForm.id ? 'update' : 'save'](Object.assign(this.templateItemForm, {template_id: this.template.id})).then(({data}) => {
         if (!go){
           ...
           this.templateItemForm.pos = this.maxLineNum;
         }
        }).finally(() => {
          this.loading = false;
        });
